...
window.tsfGBC = function( $ ) {
	...
	function maintainSEOBarTooltipPlacement() {
		const tsfBox = document.getElementById( 'tsf-inpost-box' );
		const isEditorIframed = !! document.querySelector( '.editor-visual-editor.is-iframed' );
		let tsfBoxLocation,
		    isSorting = false;

		const determineTooltipPlacement = () => {
			tsfBoxLocation = tsfBox.parentElement;
			const [ bar ] = tsfBox.getElementsByClassName( 'tsf-seo-bar' );
			const shouldUseBottomPlacement =
				isEditorIframed ||
				tsfBox.closest( '#tabs-1-edit-post\\/document-view' ) // Gutenberg sidebar WP 6.6+;
			if ( shouldUseBottomPlacement ) bar.dataset.ttPlacement = 'bottom';
			else delete bar.dataset.ttPlacement;
		}

		determineTooltipPlacement();

		const spyMetaBoxAreas = new MutationObserver( ( mutations ) => {
			if ( isSorting ) return;

			for ( const { addedNodes } of mutations ) {
				for ( const node of addedNodes ) {
					if ( node !== tsfBox ) continue;

					// Unless the location changed do nothing.
					if ( tsfBox.parentElement !== tsfBoxLocation ) {
						determineTooltipPlacement();
					}
				}
			}
		...
		const metaBoxAreas = document.querySelectorAll( '.meta-box-sortables' );
		for ( const area of metaBoxAreas ) {
			// To detect if the tsf meta box changes locations a observe child list
			// mutations in each meta box area. Sortables don’t know about changes
			// from mover buttons.
			spyMetaBoxAreas.observe( area, { childList: true } );
			// To filter inconsequential mutations listen to sortable events.
			jQuery( area ).on( 'sortstart sortstop', ( { type } ) => {
				isSorting = type === 'sortstart'
			} );
		}
	}
	...
	function _initCompat() {
		...
		subscribe( tsfUtils.debounce( sidebarDispatcher, 500 ), editPostStore );
		subscribe( tsfUtils.debounce( assessData, 300 ), editorStore );
		subscribe( saveDispatcher, editorStore );

		const endReadyCheck = subscribe(
			() => {
				const isReady = wp.data.select( editorStore ).__unstableIsEditorReady();
				if ( isReady ) {
					endReadyCheck();
					// Sure the editor said it’s ready but not all its DOM is yet. To
					// test for the iframe’s presence another event loop has to pass.
					setTimeout( maintainSEOBarTooltipPlacement );
				}
			},
			editorStore
		);
